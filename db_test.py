...
if sys.argv[1] == "clear":
    ...
    with app.app_context():
        ...
        for row_tuple in rpi_departments_rows:
            row = RPIDepartments(name=row_tuple[0], description=row_tuple[1], school_id=row_tuple[2])
            db.session.add(row)
            db.session.commit()

        lab_manager_rows = (("led", "Duy Le", "Computer Science"), ("cenzar", "Rafael", "Computer Science"))

        for row_tuple in lab_manager_rows:
            row = LabManager(rcs_id=row_tuple[0], name=row_tuple[1], department_id=row_tuple[2])
            db.session.add(row)
            db.session.commit()
